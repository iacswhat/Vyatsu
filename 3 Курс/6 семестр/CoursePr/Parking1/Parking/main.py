...
db = sqlite3.connect('server.db')
sql = db.cursor()
sql.execute("""CREATE TABLE IF NOT EXISTS parking (number TEXT, countvisit INT)""")
db.commit()
...
def save_file():
    ...
    sql.execute("""DELETE FROM parking""")
    db.commit()
    for i in range(len(arr)):
        sql.execute('''INSERT INTO parking(number, countvisit) VALUES(?, ?)''', arr[i])
        db.commit()

    for value in sql.execute("SELECT * FROM parking"):
        print(value)
