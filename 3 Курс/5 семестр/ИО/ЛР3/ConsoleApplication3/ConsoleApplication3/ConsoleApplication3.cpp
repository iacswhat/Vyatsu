...
int SimplexMethod(int n1, int m1)
{
	...
	for (int i = 1; i <= m1; i++)	//заполнение матрицы дополнительными x
	{
		matr[i][0] = i + m1;
	}
	...
	{
		std::cout << "решений нет (отрицательный базис)" << std::endl;
		return 0;
	}
	return 0;
}
...
int gomoryMethod(int n1, int m1)
{
		...
		do
		{
			...
			else
			{
				...
				if ((count == n1 && basicStringFound == true) || number == 14)
				{
					std::cout << "решение получено:" << std::endl;
					std::cout << "F = " << -matr[n1 + 1][2] << std::endl;
					for (int i = 1; i <= n1; i++)
