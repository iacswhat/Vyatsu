...
begin
for i:=0 to CountXYZ-1 do begin
MasPointsXY[i].X:=MasDeviations[i].DeviationsX*cos(p)+MasDeviations[i].DeviationsZ*sin(p);
MasPointsXY[i].Y:=MasDeviations[i].DeviationsX*sin(k)*sin(p)+MasDeviations[i].DeviationsY*cos(k) - MasDeviations[i].DeviationsZ*sin(k)*cos(p);
end;
end;
