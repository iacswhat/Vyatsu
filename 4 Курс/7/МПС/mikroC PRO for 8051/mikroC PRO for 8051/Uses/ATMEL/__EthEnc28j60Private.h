...
PARAMETERS      :
        ...
        s       : pointer to first byte of MCU RAM to compare
        l       : length in bytes of memory to compare
RETURNS         : 0 if compare matches, > 0 otherwise
DESCRIPTION     : compares ENC RAM to MCU RAM
REQUIRES        : SPI_Ethernet_init must have been called before
...
PARAMETERS      :
        ...
        s       : pointer to first byte of MCU RAM to copy to ENC
        l       : length in bytes of memory to copy
RETURNS         : nothing
DESCRIPTION     : copy MCU memory to ENC RAM.
REQUIRES        : SPI_Ethernet_init must have been called before
...
DESCRIPTION     : this is part of SPI_Ethernet_Init routine. For better linkage
                  due to MCU flash paging, initialization routine was splited
